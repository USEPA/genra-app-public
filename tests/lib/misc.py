...
from genraweb.lib.multitarget import is_multi
from tests.defs import STOCHASTIC_PATHS
...
def print_data(data):
    """Print dictionary as pretty-fied JSON."""
    logger.info(json.dumps(data, indent=4))


def process_calibration(
    fp_type: str,  # Key describing test, used to store expected result.
    data: dict,  # Current result.
    name: Path,  # Path for results for this kind of test.
) -> None:
    """Possibly update and expected result, if it's significantly different."""
    with FileLock(name.with_suffix(name.suffix + ".lock")).acquire():
        if Path(name).exists():
            ...
            expected = {}
        diff = DeepDiff(
            expected.get(fp_type, {}),
            data,
            exclude_regex_paths=STOCHASTIC_PATHS,  # includes "_changelog"
        )
        if not diff:
            logger.info(f"No change for {fp_type}")
            return
        logger.info(f"Updating {fp_type}")
        changelog = expected.get(fp_type, {}).get("_changelog", [])
        changelog[:0] = [datetime.datetime.utcnow().replace(microsecond=0).isoformat()]
        data["_changelog"] = changelog[:50]
...
def check_dict_structure(data, structure, exact_match=True, ignore=[]):
    ...
    if exact_match:
        unexpected_keys = set(data.keys()) - set(checked_keys) - set(ignore)
        assert (
            len(unexpected_keys) == 0
        ), f"Unexpectedly got {unexpected_keys} in {data}"


def check_dict_structure_bool(data, structure, **kwargs):
    """Convenience wrapper on check_dict_structure() that returns
    a boolean as opposed to AssertionError
    """
    try:
        check_dict_structure(data, structure, **kwargs)
        return True
    except AssertionError:
        return False


def check_dict_structure_one_of(data, structures, **kwargs):
...
def check_first(expected_test_params, chem_id):
    """Check the chem_id is the first result.
    Caller has already picked first result.
    """
    expect = expected_test_params.chem_id
    got = chem_id
    if is_multi(expect):
        expect = expect.split(",")[0]
    if is_multi(got):
        got = got.split(",")[0]
    expect, _ = ChemID.promote_id(expect)
    got, _ = ChemID.promote_id(got)
