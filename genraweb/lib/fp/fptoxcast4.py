...
class FPToxcast4(FPGen):
    ...
    batch_size = 1000  # 9429 records final output
    description = (
    ...
    fp_output_basename = "toxcast4_fp"
    fp_fields = [
        FPGen.FP_fields(fp_output_basename, "fpnd.all"),
    ]
    pred_fields = [
        FPGen.FP_fields(fp_output_basename, "fpnd.biop_txct"),
        FPGen.FP_fields(fp_output_basename, "fpnd.bion_txct"),
    ]
    fp_id = "bio_txct4"
    input_collection_name = "g2025_invitrodb_assay_rslt"
    maxDepType = DeployType.PROD
    name = "Biology: ToxCast 4.1 data"
    similarity_tag = "b"
    testForSmile = False
    on_the_fly = False
    ...
    def __init__(self, DB, fp_coll_name):
        ...
        self.dbc_assay = DB.g2025_toxcast_assays
        self.src = "invitrodb_v4.1"
    ...
    def generate_fps(self, chem_ids):
        ...
        for dtxsid, chem_records in groupby(records_in, lambda x: x["dsstox_sid"]):
            ...
            H["hitc"] = H["hitc"].apply(lambda x: 1 if x > 0.9 else 0)
